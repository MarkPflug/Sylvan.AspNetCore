...
namespace Benchmarks
{
	...
	public class InputFormatterBenchmarks
	{
		...
		void GenerateData()
		{
			...
			{
				var ms = new MemoryStream();
				using (var cw = ExcelDataWriter.Create(ms, ExcelWorkbookType.ExcelXml))
				{
					cw.Write("Sheet1", GenerateData(RecordCount).AsDataReader());
				}
		...
		public async Task Excel()
		{
			...
			for (int i = 0; i < IterationCount; i++)
			{
				...
				content.Headers.Add("Content-Type", "application/vnd.ms-excel");
